<nav class="navbar">
    ...
    <div class="navbar-content">
        ...
        <div class="d-flex align-items-center flex-wrap text-nowrap">
            <h4 style="color:rgb(9, 100, 9);">{{ toBikramSambatDate(\Carbon\Carbon::today()) }} <span
                    style="color:#df800b">{{ \Carbon\Carbon::now()->format('l') }}</span></h4>
        </div>
        <ul class="navbar-nav">
            <li class="nav-item">
            ...
            <li class="nav-item dropdown">
                <a class="nav-link dropdown-toggle" href="#" id="profileDropdown" role="button"
                    ...
                    <img class="wd-30 ht-30 rounded-circle"
                        src="{{ Auth::user()->avatar ? asset('/avatars/thumbnail/' . Auth::user()->avatar) : (Auth::user()->gender == 'female' ? asset('/avatar/female-avatar.jpg') : asset('/avatar/male-avatar.jpg')) }}"
                        style="object-fit:cover;object-position:cover;" alt="profile">
                </a>
...
@push('scripts')
    <script>
        function calculateTimeDifference(previousTime) {
            var kathmanduTimeZone = 'Asia/Kathmandu';
            var currentTime = new Date();
            var currentKathmanduTime = new Date(currentTime.toLocaleString('en-US', {
                timeZone: kathmanduTimeZone
            }));

            var previousDateTime = new Date(currentKathmanduTime.toDateString() + ' ' + previousTime);

            var timeDifference = Math.abs(currentKathmanduTime - previousDateTime);
        ...
        function updateTimer(previousTime) {
            ...
            updateDisplay(); // Update immediately before starting the interval
