                        ...
                        <div class="col-12 mb-2 ">
                            ...
                            <select class="form-control permission-select" id="permission-select" multiple="multiple"
                                ...
                                @foreach ($permissions as $permission)
                                    <option {{ isset($selectedPermissions)? in_array($permission->id, $selectedPermissions) ? 'selected' : '':'' }} value="{{ $permission->id }}">{{ $permission->title }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-12 mb-2 ">
                            ...
                            <select name="designation" class="form-control">
                                ...
                                @foreach (config("constants.designations") as $designation)
                                    <option {{ (isset($user) && $user->designation == $designation) ? "selected" : '' }}
                                        value="{{ $designation }}">{{ ucwords(str_replace('-', ' ', $designation)) }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-12 mb-2 ">
                            ...
                            <select name="employment_type" class="form-control">
                                ...
                                @foreach (config("constants.employment_types") as $type)
                                    <option {{ (isset($user) && $user->employment_type == $type) ? "selected" : '' }}
                                        value="{{ $type }}">{{ ucwords(str_replace('-', ' ', $type)) }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-12 mb-2 ">
